...
const POST_URL           = process.env.POST_URL;             // Endpoint externo principal
const ALT_POST_URL       = process.env.ALT_POST_URL;         // Endpoint externo alternativo
 ...
 */
async function postToExternalQueue(url, enrichedMessage) {
  ...
  receiver.subscribe({
    async processMessage(message) {
      ...
      try {
        ...
        console.log('Construcción del payload');
        ...
        console.log('Pre publicación en la cola');

        // 3. Determinar la URL de publicación según failOn
        const url = (message.body.failOn === 'microservice2') ? ALT_POST_URL : POST_URL;

        // 4. Publicar en el endpoint externo
        const status = await postToExternalQueue(url, enriched);
